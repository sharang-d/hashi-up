...
download_and_install() {
  if [ -f "${TMP_DIR}/boundary.zip" ]; then
    ...
    if [ -x "${BIN_DIR}/boundary" ] && [ "$(${BIN_DIR}/boundary version | grep "Version Number" | tr -s ' ' | cut -d' ' -f4)" = "${BOUNDARY_VERSION}" ]; then
      ...
      info "Downloading and unpacking boundary_${BOUNDARY_VERSION}_linux_${SUFFIX}.zip"
      curl -o "$TMP_DIR/boundary.zip" -sfL "https://releases.hashicorp.com/boundary/${BOUNDARY_VERSION}/boundary_${BOUNDARY_VERSION}_linux_${SUFFIX}.zip"
      $SUDO unzip -qq -o "$TMP_DIR/boundary.zip" -d $BIN_DIR
    fi
  fi
}
...
init_database() {
  [ "${INIT_DATABASE}" = false ] && return

  $SUDO ${BIN_DIR}/boundary database init -config ${BOUNDARY_CONFIG_DIR}/boundary.hcl
}
